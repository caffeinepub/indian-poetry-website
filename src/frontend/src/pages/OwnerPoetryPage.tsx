...
import { useMainPoet } from '../hooks/useQueries';
...
import { ArrowLeft, Award, BookOpen, Crown, Globe } from 'lucide-react';
import PoetrySectionCard from '../components/PoetrySectionCard';
import { ContentCategory } from '../backend';

export default function OwnerPoetryPage() {
  const navigate = useNavigate();
  const { t } = useLanguage();
  const { data: poet, isLoading, error } = useMainPoet();
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      {/* Owner Header - Distinguished Design */}
      <Card className="mb-8 bg-gradient-to-br from-primary/10 via-card to-accent/20 border-2 border-primary/30">
        <CardContent className="p-8">
          <div className="flex items-center gap-3 mb-6">
            <Crown className="w-8 h-8 text-primary" />
            <h1 className="text-3xl font-bold text-primary">{t('ownerPoetry')}</h1>
          </div>

          <div className="grid md:grid-cols-4 gap-8">
            <div className="flex flex-col items-center">
              <Avatar className="w-40 h-40 border-4 border-primary/40">
            ...
            <div className="md:col-span-3 space-y-4">
              <div>
                <h2 className="text-4xl font-bold text-foreground mb-2">{poet.name}</h2>
                {poet.website && (
                  <a
                    href={poet.website}
                    target="_blank"
              ...
              {poet.biography && (
                <div>
                  <h3 className="text-xl font-semibold mb-2">{t('biography')}</h3>
